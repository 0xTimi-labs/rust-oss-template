...
| Workflow | 触发 | 内容 |
|---|---|---|
| `ci.yml` | PR / push main / merge queue / 每周一 | 三平台测试矩阵、fmt、clippy、覆盖率上报、E2E、前端构建、安全门禁（gitleaks、cargo-deny、CodeQL），main 上跑性能基准 |
| `security.yml` | 由 `ci.yml` 复用调用（workflow_call） | gitleaks 密钥扫描、cargo-deny 依赖检查、CodeQL 静态分析 |
